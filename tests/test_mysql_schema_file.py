...
class MysqlSchemaFileTest(unittest.TestCase):
    def test_schema_file_contains_required_tables(self) -> None:
        ...
        self.assertIn("create table if not exists workspaces", text)
        self.assertIn("create table if not exists agent_states", text)
        self.assertIn("create table if not exists chat_sessions", text)
        self.assertIn("create table if not exists chat_messages", text)
        self.assertIn("create table if not exists requirements", text)
        self.assertIn("create table if not exists session_requirements", text)
        ...
        self.assertIn("unique key uk_workspaces_workspace_id (workspace_id)", text)
        ...
        self.assertIn("comment='工作区元数据表'".lower(), text)
    ...
    def test_migration_script_exists_and_mentions_comments(self) -> None:
        ...
        self.assertIn("mysql -h127.0.0.1", text)
